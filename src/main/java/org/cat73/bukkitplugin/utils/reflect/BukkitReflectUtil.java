...
public class BukkitReflectUtil extends ReflectUtil {
    ...
    public static Class<?> getCraftBukkitClass(String path) throws Exception {
        // 拼凑完整 Path
        path = String.format("org.bukkit.craftbukkit.%s.%s", BukkitReflectUtil.BUKKIT_VERSION, path);
        // 获取并返回 Class
    ...
    public static Class<?> getMinecraftServerClass(String path) throws Exception {
        // 拼凑完整 Path
        path = String.format("net.minecraft.server.%s.%s", BukkitReflectUtil.BUKKIT_VERSION, path);
        // 获取并返回 Class
        return Class.forName(path);
    }
}
